...
export function initializeDB() {
  ...
  if (fs.existsSync(DB_PATH)) {
    try {
      ...
      db = { stocks: defaultStocks, transactions: defaultTransactions, reports: defaultReports };
    }
  } else {
    db = { stocks: defaultStocks, transactions: defaultTransactions, reports: defaultReports };
    saveToDB();
...
// Ensure database is initialized at file compile time too
initializeDB();
...
// 2. CREATE STOCK
apiRouter.post('/api/stock', (req, res) => {
...
// 3. UPDATE STOCK
apiRouter.put('/api/stock/:id', (req, res) => {
  const { id } = req.params;
...
// 4. DELETE STOCK
apiRouter.delete('/api/stock/:id', (req, res) => {
  const { id } = req.params;
...
apiRouter.post('/api/finance', (req, res) => {
...
apiRouter.delete('/api/finance/:id', (req, res) => {
  const { id } = req.params;
...
// 7. GENERATE REPORT VIA GEMINI
apiRouter.post('/api/report/generate', async (req, res) => {
  try {
...
Berikan judul laporan yang menarik, letakkan tanggal laporan saat ini (2026-05-31), dan sapa jajaran Direksi PT Kalimaya Alunna Indonesia dengan hormat.`;
